...
export default function Categories() {
  ...
  return (
    <div className="container mx-auto px-4">
      <ScrollReveal>
        <h2 className="mb-8 text-2xl font-bold text-gray-900 md:text-3xl">Shop by Category</h2>
      </ScrollReveal>

      <div className="grid gap-6 sm:grid-cols-2 md:grid-cols-4">
        {categories.map((category, index) => (
          <ScrollReveal key={category.name} delay={index * 100} direction={index % 2 === 0 ? "left" : "right"}>
            <Link
              href={category.href}
