  ...
  return (
    <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
      {items.map((item, index) => (
        <ParallaxCard key={index} {...item} disabled={isMobile} delay={index * 100} />
      ))}
    </div>
  )
}
...
function ParallaxCard({ title, description, image, link, disabled = false, delay = 0 }: ParallaxCardExtendedProps) {
